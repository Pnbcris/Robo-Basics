...
private final XRPMotor m_XRPleftMotor = new XRPMotor(0);
private final XRPMotor m_XRPrightMotor = new XRPMotor(1);
...
private final XboxController m_XRPController = new XboxController(2);
  ...
  public void teleopInit() {
    //occurs only when robot is in teleop mode
    m_XRPrightMotor.setInverted(true);
  }

  @Override
  public void teleopPeriodic() {
    double forward = -m_XRPController.getLeftY();
    double turn = -m_XRPController.getRightY();

    m_XRPleftMotor.set(forward - turn);
    m_XRPrightMotor.set(forward + turn);
